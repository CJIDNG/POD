              ...
              <div v-if="posts.length">
                    ...
                    <div class="card shadow bg-transparent">
                      ...
                      <div class="card-body pt-0 pb-2">
                        <ul>
                          ...
                          <li>{{ submittedCount }} {{ trans.app.submitted }}</li>
                          <li>{{ approvedCount }} {{ trans.app.approved }}</li>
                        </ul>
                      </div>
                    </div>
                </div>
                ...
                <div class="mt-5">
                  <div v-for="(post, $index) in posts" :key="$index" class="d-flex border-top py-3 align-items-center">
                    <div class="mr-auto">
                      <p class="mb-1 mt-2">
                        <router-link :to="{name: 'posts-stats-show', params: { id: post.id }}" class="font-weight-bold text-lg lead text-decoration-none">
                          {{ post.title }}
                        </router-link>
                      </p>
                      <p class="text-muted mb-2">
                        ...
                        ―
                        <router-link :to="{name: 'posts-stats-show', params: { id: post.id }}" class="text-decoration-none text-muted">
                    ...
                    <div class="ml-auto d-none d-lg-block">
                      ...
                      {{ trans.app.created }} {{ moment(post.created_at).locale(CurrentTenant.locale).fromNow() }}
                    </div>
                  </div>
                </div>
            ...
            fetchStats() {
              this.request()
                ...
                .then(response => {
                  ...
                  this.submittedCount = response.data.submitted_count
                  this.approvedCount = response.data.approved_count
