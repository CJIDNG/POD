    ...
    <template slot="main">
      <page-header>
        <template slot="action">
          <router-link
            to="/stats"
            class="btn btn-sm btn-outline-success font-weight-bold my-auto ml-auto"
          >{{ trans.app.see_all_stats }}</router-link>
        </template>
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right">
              <router-link
                ...
                class="dropdown-item"
              >{{ trans.app.edit_post }}</router-link>
            </div>
          </div>
        </template>
      </page-header>
        ...
        <div class="col-xl-10 offset-xl-1 px-xl-5 col-md-12">
          ...
          <div class="row justify-content-start mt-3">
            <div class="col-lg-6 col-12">
              ...
              <div class="d-flex">
                <div class="mr-5">
                  <p
                    class="mb-0 small text-muted text-uppercase font-weight-bold"
                  >{{ trans.app.total_views }}</p>
                  <h3 class="mt-1">{{ suffixedNumber(viewCountLifetime) }}</h3>
                </div>

                <div>
                  <p
                    class="mb-0 small text-muted text-uppercase font-weight-bold"
                  >{{ trans.app.average_reading_time }}</p>
                  <h3 class="mt-1">{{ readTime }}</h3>
                </div>
              </div>
            </div>
              ...
              <div class="d-flex">
                <div class="mr-5">
                  <p
                    class="mb-0 small text-muted text-uppercase font-weight-bold"
                  >{{ trans.app.views }}</p>
                  <h3 class="my-1">{{ suffixedNumber(viewCount) }}</h3>
                ...
                <div>
                  ...
                  <h3 class="my-1">{{ suffixedNumber(visitCount) }}</h3>
                  <p class="text-muted text-md-right">
          ...
          <div class="row justify-content-between">
            <div class="col-md-6 mt-4">
              <h5
                class="text-muted small text-uppercase font-weight-bold border-bottom pb-2"
              >{{ trans.app.views_by_traffic_source }}</h5>

              <div v-if="traffic">
                <div v-for="(views, host, index) in traffic" :key="index">
                      ...
                      <div v-if="host === trans.app.other">
                        <p class="mb-0 py-1">
                          <img
                            ...
                            :style="CurrentTenant.darkMode === true ? {filter: 'invert(100%)'} : ''"
                            :alt="host"
                            class="mr-1"
                          />
                          <a
              ...
              <p v-else class="py-2 font-italic">{{ trans.app.waiting_until_more_data }}</p>
            </div>

            <div class="col-md-6 mt-4">
              <h5
                class="text-muted small text-uppercase font-weight-bold border-bottom pb-2"
              >{{ trans.app.popular_reading_times }}</h5>

              <div v-if="popularReadingTimes">
                <div v-for="(percentage, time, index) in popularReadingTimes" :key="index">
                  <div class="d-flex py-2 align-items-center">
                    <div class="mr-auto">
                      <p class="mb-0 py-1">{{ time }}</p>
                    </div>
                    <div class="ml-auto">
    ...
    next(vm => {
      vm.request()
        ...
        .then(response => {
          ...
          vm.popularReadingTimes = Array.isArray(
            response.data.popular_reading_times
          )
            ? null
            : response.data.popular_reading_times;
          vm.readTime = response.data.read_time;
          vm.viewMonthOverMonthDirection =
            response.data.view_month_over_month.direction;
          vm.viewMonthOverMonthPercentage =
            response.data.view_month_over_month.percentage;
          vm.visitMonthOverMonthDirection =
            response.data.visit_month_over_month.direction;
          vm.visitMonthOverMonthPercentage =
            response.data.visit_month_over_month.percentage;
