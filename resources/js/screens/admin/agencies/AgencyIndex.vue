    ...
    <template slot="main">
      <page-header>
        <template slot="action">
          <router-link
            :to="{ name: 'agencies-create' }"
            class="btn btn-sm btn-outline-success font-weight-bold my-auto"
          >{{ trans.app.new_agency }}</router-link>
        </template>
      </page-header>
          ...
          <div class="mt-2">
            ...
            >
              <div class="mr-auto">
                <p class="mb-0 py-1">
                  <router-link
                    :to="{name: 'agencies-edit', params: { id: agency.id }}"
                    class="font-weight-bold text-lg lead text-decoration-none"
                  >{{ agency.name }}</router-link>
                </p>
              </div>
              <div class="ml-auto">
                <span class="text-muted mr-3">{{ agency.government_projects_count }} {{ trans.app.governmentProjects }}</span>
                <span
                  class="d-none d-md-inline-block"
                >{{ trans.app.created }} {{ moment(agency.created_at).locale(CurrentTenant.locale).fromNow() }}</span>
              </div>
            </div>
