    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header d-flex align-items-center justify-content-between border-0">
          <h4 class="modal-title">{{ trans.app.publishing }}</h4>
        ...
        <div class="modal-body">
          <p class="text-muted">
            {{ trans.app.product_scheduling_format }}
            <span class="font-weight-bold">{{ CurrentTenant.timezone }}</span>
          ...
          <div class="row">
            ...
            >
              <div class="d-flex align-items-center">
                ...
                >
                  <option
                    v-for="(value, index) in Array.from({ length: 12 }, (_, i) => String(i + 1).padStart(2, '0'))"
                    :value="value"
                    :key="index"
                  >{{ value }}</option>
                </select>
                ...
                >
                  <option
                    v-for="(value, index) in Array.from({ length: 31 }, (_, i) => String(i + 1).padStart(2, '0'))"
                    :value="value"
                    :key="index"
                  >{{ value }}</option>
                </select>
                ...
                >
                  <option
                    v-for="(value, index) in Array.from({ length: 15 }, (_, i) => i + new Date().getFullYear() - 10)"
                    :value="value"
                    :key="index"
                  >{{ value }}</option>
                </select>
            ...
            >
              <div class="d-flex align-items-center">
                ...
                >
                  <option
                    v-for="(value, index) in Array.from({ length: 24 }, (_, i) => String(i).padStart(2, '0'))"
                    :value="value"
                    :key="index"
                  >{{ value }}</option>
                </select>
                ...
                >
                  <option
                    v-for="(value, index) in Array.from({ length: 60 }, (_, i) => String(i).padStart(2, '0'))"
                    :value="value"
                    :key="index"
                  >{{ value }}</option>
                </select>
          ...
          <p
            ...
            v-if="isScheduled"
          >{{ trans.app.your_product_will_publish_at }} {{ moment(this.activeProduct.published_at).format('h:mm A') }} {{ trans.app.on }} {{ moment(this.activeProduct.published_at).format('MMMM DD, YYYY') }}.</p>
        </div>
          ...
          <div class="row w-100">
            ...
            <div class="col-lg order-lg-first px-0">
              <button
                ...
                data-dismiss="modal"
              >{{ trans.app.cancel_scheduling }}</button>
...
export default {
  ...
  computed: {
    ...
    isScheduled() {
      return (
        this.activeProduct.published_at &&
  ...
  methods: {
    ...
    scheduleOrPublish() {
      this.activeProduct.published_at = this.result;
